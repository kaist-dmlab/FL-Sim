...
class Model(AbstractModel):
    ...
    def get_logits(self, x, W_0, b_0, W_1, b_1):
    ...
    def createModel(self):
        ...
        W_1 = tf.get_variable('W_1', dtype=tf.float32, initializer=tf.random_normal([50, self.numClasses], stddev=(1/tf.sqrt(float(50)))))
        b_1 = tf.get_variable('b_1', dtype=tf.float32, initializer=tf.random_normal([self.numClasses]))
        
        y_onehot = tf.one_hot(self.y, self.numClasses)
        logits = self.get_logits(self.x, W_0, b_0, W_1, b_1)
