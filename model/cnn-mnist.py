...
class Model(AbstractModel):
    ...
    def get_logits(self, x, W_conv1, b_conv1, W_conv2, b_conv2, W_fc1, b_fc1, W_fc2, b_fc2):
    ...
    def createModel(self):
        ...
        W_fc2 = tf.get_variable('W_fc2', dtype=tf.float32, initializer=tf.truncated_normal(shape=[256, self.numClasses], stddev=5e-2))
        b_fc2 = tf.get_variable('b_fc2', dtype=tf.float32, initializer=tf.constant(0.0, shape=[self.numClasses]))
        
        y_onehot = tf.one_hot(self.y, self.numClasses)
        logits = self.get_logits(self.x, W_conv1, b_conv1, W_conv2, b_conv2, W_fc1, b_fc1, W_fc2, b_fc2)
