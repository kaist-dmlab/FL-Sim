...
#         b_fc2 = tf.get_variable('b_fc2', dtype=tf.float32, initializer=tf.constant(0.0, shape=[10]))
    ...
    def getInitVars(self):
        ...
        W_conv1_ = np.random.normal(0, 5e-2, 5*5*1*64).reshape((5, 5, 1, 64)).astype(np.float32)
        ...
        W_fc1_ = np.random.normal(0, 5e-2, 1024*256).reshape((1024, 256)).astype(np.float32)
